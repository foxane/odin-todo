...
const timePeriodBtns = document.querySelectorAll(".category-btn");
...
newTaskBtn.addEventListener("click", () => {
  if (Project.projectList.length === 0) {
    ...
    dialog.innerHTML = "";
    dialog.appendChild(modal.createProjectForm());
    dialog.showModal();
    return;
  }
  dialog.innerHTML = "";
  dialog.appendChild(modal.createTask());
  dialog.showModal();
});
...
const updateUi = (() => {
  ...
  const sortTasks = (taskArr) => {
    ...
    if (sortVal === "priority") {
      ...
      for (const task of taskArr) {
        tasksView.appendChild(elementFactory.createTask(task));
      }
  ...
  const project = () => {
    ...
    projectsView.appendChild(elementFactory.createAllProjectBtn());
    for (const project of Project.projectList) {
      projectsView.appendChild(elementFactory.createProjectBtn(project));
    }
  };
...
const elementFactory = (() => {
  ...
  const editTask = (task) => {
    ...
    dialog.appendChild(modal.editTask(task));
    dialog.showModal();
    // Update localStorage
    updateLocalStorage();
  };
...
const modal = (() => {
  ...
  const createTaskForm = (dialogTitle, type, task) => {
    // Create form element
    const form = document.createElement("form");
    form.setAttribute("action", "");
    form.setAttribute("method", "dialog");
  ...
  return { createTask, editTask, createTaskForm };
})();
